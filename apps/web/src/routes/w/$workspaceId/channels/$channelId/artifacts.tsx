...
const ChannelArtifacts = () => {
  ...
  if (artifacts.isPending) return <div className="flex flex-col gap-2">{[0, 1, 2].map((i) => <Skeleton className="h-20 w-full rounded-xl" key={i} />)}</div>;
  ...
  return <div className="flex flex-col gap-2">{artifacts.data.map((artifact) => {
    ...
    return <Link className="flex flex-col gap-2 rounded-xl border border-border bg-card p-4 transition-colors hover:bg-muted/40" key={artifact.id} params={{ artifactId: artifact.id, workspaceId }} to="/w/$workspaceId/library/$artifactId"><div className="flex items-start gap-2"><Glyph aria-hidden="true" className="mt-0.5 size-4 shrink-0 text-muted-foreground" /><span className="truncate text-foreground text-sm leading-snug">{artifact.name}</span></div><div className="flex items-center gap-2 pl-6 text-muted-foreground text-xs"><span className="truncate">{artifact.contentType.split(";")[0]}</span><span aria-hidden="true">·</span><span>{formatBytes(artifact.byteLength)}</span><span aria-hidden="true">·</span><span>updated {relativeTime(artifact.updatedAt)}</span></div></Link>;
  })}</div>;
};
