...
or multiplayer? The structural twist (a channel _is_ an agent, not a human chatroom) already
...
- **Single-player threads are the focus.** Each member has their **own private** conversations
  (threads) with a channel's agent. The shared asset is the _agent_ (the shape); the private
...
- **"Multiplayer" = shared workspace assets, not multiple humans in a thread.** Members join a
  ...
  documents**. Collaboration happens at the _asset_ level (agents + documents), not in-thread.
...
- Multiplayer-thread seam: ThreadAgent stream fan-out + per-thread human presence must not be
  designed _out_, but remain unbuilt.
